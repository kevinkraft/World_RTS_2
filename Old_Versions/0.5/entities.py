...
class Unit(Entity_Action):
     ...
     def DumpInventory(self):
          ...
          #I can add a 'special item' or 'enhancement', or just 'resource' atribute to item so that this function 
          # doesn't throw away the things we will want to keep in the inv (like weapons)

          stockpile = self.stockpile
          #save initial position
          initpos = self.pos
          #do move back to initial position
          self.MoveTo(initpos, prepend_option = True) #I think this is unnecessary in all situations
...
def display_construction_atributes(Construction_list):
     ...
     for construction_ in Construction_list:
          print "|  {0}  |  [{1:.2f},{2:.2f}]  |   {3}   |  {4:.2f}  | ".format(construction_.name, construction_.pos[0], construction_.pos[1], construction_.type_, construction_.work)
          print construction_.materials
